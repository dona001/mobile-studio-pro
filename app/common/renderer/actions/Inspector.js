...
// Recording Save & Replay Action Types
export const SAVE_RECORDING_REQUESTED = 'SAVE_RECORDING_REQUESTED';
export const SAVE_RECORDING_SUCCESS = 'SAVE_RECORDING_SUCCESS';
export const SAVE_RECORDING_ERROR = 'SAVE_RECORDING_ERROR';
export const GET_SAVED_RECORDINGS_REQUESTED = 'GET_SAVED_RECORDINGS_REQUESTED';
export const GET_SAVED_RECORDINGS_SUCCESS = 'GET_SAVED_RECORDINGS_SUCCESS';
export const GET_SAVED_RECORDINGS_ERROR = 'GET_SAVED_RECORDINGS_ERROR';
export const DELETE_RECORDING_REQUESTED = 'DELETE_RECORDING_REQUESTED';
export const DELETE_RECORDING_SUCCESS = 'DELETE_RECORDING_SUCCESS';
export const DELETE_RECORDING_ERROR = 'DELETE_RECORDING_ERROR';
export const START_REPLAY_REQUESTED = 'START_REPLAY_REQUESTED';
export const START_REPLAY_SUCCESS = 'START_REPLAY_SUCCESS';
export const UPDATE_REPLAY_PROGRESS = 'UPDATE_REPLAY_PROGRESS';
export const REPLAY_COMPLETED = 'REPLAY_COMPLETED';
...
// Recording Save & Replay Actions
export function saveRecording(name, description) {
  return async (dispatch, getState) => {
    const {inspector} = getState();
    const {recordedActions, serverDetails, sessionCaps} = inspector;

    if (!recordedActions.length) {
      return dispatch({type: SAVE_RECORDING_ERROR, error: 'No actions to save'});
    }

    try {
      dispatch({type: SAVE_RECORDING_REQUESTED});
      ...
      const savedRecordings = (await getSetting(SAVED_RECORDINGS)) || [];
      savedRecordings.push(recording);
      await setSetting(SAVED_RECORDINGS, savedRecordings);

      dispatch({type: SAVE_RECORDING_SUCCESS, recording});
      dispatch({type: GET_SAVED_RECORDINGS_SUCCESS, recordings: savedRecordings});
    } catch (error) {
      dispatch({type: SAVE_RECORDING_ERROR, error: error.message});
    }
  };
}
...
export function getSavedRecordings() {
  return async (dispatch) => {
    try {
      dispatch({type: GET_SAVED_RECORDINGS_REQUESTED});
      const recordings = (await getSetting(SAVED_RECORDINGS)) || [];
      dispatch({type: GET_SAVED_RECORDINGS_SUCCESS, recordings});
    } catch (error) {
      dispatch({type: GET_SAVED_RECORDINGS_ERROR, error: error.message});
    }
  };
}
...
export function deleteRecording(recordingId) {
  return async (dispatch) => {
    try {
      dispatch({type: DELETE_RECORDING_REQUESTED});
      const savedRecordings = (await getSetting(SAVED_RECORDINGS)) || [];
      const updatedRecordings = savedRecordings.filter((r) => r.id !== recordingId);
      await setSetting(SAVED_RECORDINGS, updatedRecordings);
      dispatch({type: DELETE_RECORDING_SUCCESS, recordingId});
      dispatch({type: GET_SAVED_RECORDINGS_SUCCESS, recordings: updatedRecordings});
    } catch (error) {
      dispatch({type: DELETE_RECORDING_ERROR, error: error.message});
    }
  };
}
...
export function startReplay(recordingId) {
  return async (dispatch, getState) => {
    try {
      dispatch({type: START_REPLAY_REQUESTED, recordingId});
      ...
      if (!currentState.inspector.driver) {
        throw new Error('No active session found. Please start a session before replaying.');
      }

      dispatch({type: START_REPLAY_SUCCESS, recording});

      // Execute the recording actions
      for (let i = 0; i < recording.actions.length; i++) {
        const action = recording.actions[i];
        dispatch({
          type: UPDATE_REPLAY_PROGRESS,
          currentStep: i + 1,
          totalSteps: recording.actions.length,
        });

        try {
          // For replay, we need to handle element actions differently
          // since element IDs from the original recording won't exist in the current session
          if (action.action === 'findAndAssign') {
            // Re-execute the findAndAssign to locate the element in the current session
            await dispatch(
              applyClientMethod({
                methodName: 'findAndAssign',
                args: action.params,
              }),
            );
          } else if (
            action.action === 'elementClick' ||
            action.action === 'elementSendKeys' ||
            action.action === 'elementClear'
          ) {
            // For element actions, we need to use the most recently found element
            // This is a simplified approach - in a real implementation, you'd want to
            // maintain a mapping of element variables to their current IDs
            const currentState = getState();
            const driver = currentState.inspector.driver;

            if (driver) {
              // Import InspectorDriver to access the element cache
              const InspectorDriver = (await import('../lib/appium/inspector-driver.js')).default;
              const inspectorDriver = InspectorDriver.instance(driver);

              if (inspectorDriver.elementCache) {
                // Find the most recent element in the cache
                const elementIds = Object.keys(inspectorDriver.elementCache);
                if (elementIds.length > 0) {
                  const latestElementId = elementIds[elementIds.length - 1];
                  await dispatch(
                    applyClientMethod({
                      methodName: action.action,
                      args: [latestElementId, ...action.params.slice(1)],
                    }),
                  );
                } else {
                  console.warn('No elements found in cache for replay action:', action.action);
                  // Skip this action and continue
                  continue;
                }
              } else {
                console.warn('No element cache available for replay action:', action.action);
                continue;
              }
            } else {
              console.warn('No driver available for replay action:', action.action);
              continue;
            }
          } else {
          ...
          console.warn(`Failed to execute action ${action.action}:`, actionError.message);
          // Continue with next action instead of stopping the entire replay
        }
      ...
      dispatch({type: REPLAY_COMPLETED});
    } catch (error) {
      dispatch({type: REPLAY_ERROR, error: error.message});
    }
  };
}
