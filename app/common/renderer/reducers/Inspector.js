...
import {
  ...
  DELETE_RECORDING_ERROR,
  DELETE_RECORDING_REQUESTED,
  ...
  GET_SAVED_RECORDINGS_ERROR,
  GET_SAVED_RECORDINGS_REQUESTED,
  GET_SAVED_RECORDINGS_SUCCESS,
  HIDE_GESTURE_ACTION,
  HIDE_GESTURE_EDITOR,
  ...
  START_RECORDING,
  START_REPLAY_REQUESTED,
  START_REPLAY_SUCCESS,
  STOP_REPLAY,
...
const INITIAL_STATE = {
  ...
  // Recording Save & Replay State
  savedRecordings: [],
  isSavingRecording: false,
  saveRecordingError: null,
  isLoadingRecordings: false,
  recordingsError: null,
  isReplaying: false,
  currentReplayRecording: null,
  replayProgress: {currentStep: 0, totalSteps: 0},
  replayError: null,
};
...
export default function inspector(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    // Recording Save & Replay Cases
    case SAVE_RECORDING_REQUESTED:
      return {...state, isSavingRecording: true, saveRecordingError: null};
    ...
    case SAVE_RECORDING_SUCCESS:
      return {
        ...
        isSavingRecording: false,
        saveRecordingError: null,
        savedRecordings: [...state.savedRecordings, action.recording],
      };

    case SAVE_RECORDING_ERROR:
      return {...state, isSavingRecording: false, saveRecordingError: action.error};

    case GET_SAVED_RECORDINGS_REQUESTED:
      return {...state, isLoadingRecordings: true, recordingsError: null};

    case GET_SAVED_RECORDINGS_SUCCESS:
      return {
        ...state,
        isLoadingRecordings: false,
        recordingsError: null,
        savedRecordings: action.recordings,
      };

    case GET_SAVED_RECORDINGS_ERROR:
      return {...state, isLoadingRecordings: false, recordingsError: action.error};

    case DELETE_RECORDING_REQUESTED:
      return {...state, isLoadingRecordings: true};

    case DELETE_RECORDING_SUCCESS:
      return {
        ...state,
        isLoadingRecordings: false,
        savedRecordings: state.savedRecordings.filter((r) => r.id !== action.recordingId),
      };

    case DELETE_RECORDING_ERROR:
      return {...state, isLoadingRecordings: false, recordingsError: action.error};

    case START_REPLAY_REQUESTED:
      return {
        ...state,
        isReplaying: true,
        replayError: null,
    ...
    case START_REPLAY_SUCCESS:
      return {
        ...state,
        currentReplayRecording: action.recording,
        replayProgress: {currentStep: 0, totalSteps: action.recording.actions.length},
      };
